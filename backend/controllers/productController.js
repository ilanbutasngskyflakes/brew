...
// -------------------- GET ALL PRODUCTS with ingredient-based stock --------------------
exports.getProducts = async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT 
...
// -------------------- GET PRODUCTS WITH VARIANTS AND CALCULATED STOCK --------------------
exports.getProductsFull = async (req, res) => {
  const conn = await db.getConnection();
  ...
  try {
    ...
    // Transform into nested structure with calculated stock
    const products = [];
    const productMap = new Map();
    ...
    // Calculate stock for each variant based on ingredients
    products.forEach(product => {
      product.variants.forEach(variant => {
        if (variant.ingredients.length > 0) {
          ...
          // The limiting ingredient determines the stock
          variant.quantity = Math.min(...possibleUnits);
        } else {
          // No ingredients defined, set stock to 0
          variant.quantity = 0;
        }
      });
    });
