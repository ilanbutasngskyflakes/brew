...
export const addVariant = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      "INSERT INTO variants (product_id, name, quantity, price, status) VALUES (?, ?, ?, ?, ?)",
...
export const updateVariant = async (req, res) => {
  try {
    ...
    await db.query(
      "UPDATE variants SET name=?, quantity=?, price=?, status=? WHERE id=?",
...
export const getVariantsByProduct = async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await db.query("SELECT * FROM variants WHERE product_id=?", [id]);
    res.json(rows);
  } catch (err) {
    console.error(err);
